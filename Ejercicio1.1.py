...
while menu:
    ...
    if option == "2":
        ...
        if option2 == "1":
            ...
            search_method = input("Search by (1) ID or (2) Title? ")
            if search_method == "1":
                book_search = int(input("Enter the ID of the book you would like to see: "))
                print(Library.get(book_search, "This item is not in our library."))
            elif search_method == "2":
                title_search = input("Enter the title of the book: ").lower()
                found = False
                for key, item in Library.items():
                    if item["title"].lower() == title_search:
                        print(f"ID: {key} -------------------- Book: {item}")
                        found = True
                if not found:
                    print("This title is not in our library.")
    if option == "3":
        print("====================================")
        for key, item in Library.items():
                print(f"ID: {key} -------------------- Book: {item}")
